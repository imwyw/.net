...
namespace DAL
{
    public class MySqlHelper
    {
        ...
        /// <summary>
        /// 连接字符串
        /// （注意：需要当前DAL类库引用System.Configuration程序集）
        /// （注意：需要应用程序的UI层根目录下的App.config或web.config文件中添加appSettings节点，并添加子节点 add key="constring" value="此处写连接字符串"）
        /// </summary>
            ...
            CommandType cmdType = CommandType.Text) where T : class, new()
        {
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    {
                        //默认当做属性名称和表字段名称对应，后期可以通过自定义特性进行匹配
                        //modified by wangyaunwei 2017-10-25 增加对reader取出为null值的判断，将dbnull转换为null
                        object obj = reader[p.Name] == DBNull.Value ? null : reader[p.Name];
