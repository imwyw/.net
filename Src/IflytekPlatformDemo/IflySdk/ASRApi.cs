                    ...
                    for (int i = 0; i < data.Length; i += _frameSize)
                    {
                        ...
                        switch (_status)
                        {
                            case FrameState.First:
                                ...
                                await _ws.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(firstFrame)))
                            ...
                            case FrameState.Continue:  //中间帧
                                ...
                                await _ws.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(continueFrame)))
                                    , WebSocketMessageType.Text
                                    , true
                                    , CancellationToken.None);
                                break;
                            case FrameState.Last:    // 最后一帧音频
                                ...
                                await _ws.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(lastFrame)))
                                    , WebSocketMessageType.Text
                                    , true
                                    , CancellationToken.None);
                                break;
